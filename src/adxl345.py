...
def _axes_map_to_transform(axes_map):
    transform = []
    for axis_pos, axis_scale in axes_map:
        row = [0., 0., 0.]
        row[axis_pos] = axis_scale
        transform.append(tuple(row))
...
def read_axes_transform(config, scale_x, scale_y, scale_z):
    axes_transform = _axes_map_to_transform(
        read_axes_map(config, scale_x, scale_y, scale_z))
    axes_rotation = config.getfloatlist('axes_rotation', (0., 0., 0.), count=3)
    if not any(axes_rotation):
        return axes_transform
    rotation_transform = _build_rotation_transform(axes_rotation)
    return _matrix_multiply(rotation_transform, axes_transform)

def apply_axes_transform(raw_xyz, axes_transform):
    return tuple([
        round(sum([axis_value * coeff
                   for axis_value, coeff in zip(raw_xyz, axis_coeffs)]), 6)
...
class ADXL345:
    def __init__(self, config):
        ...
        self.axes_transform = read_axes_transform(config, SCALE_XY, SCALE_XY,
                                                  SCALE_Z)
    ...
    def _convert_samples(self, samples):
        ...
        for ptime, xlow, ylow, zlow, xzhigh, yzhigh in samples:
            ...
            rz = ((zlow | ((xzhigh & 0xe0) << 3) | ((yzhigh & 0xe0) << 6))
                  - ((yzhigh & 0x40) << 7))
            raw_xyz = (rx, ry, rz)
            x, y, z = apply_axes_transform(raw_xyz, self.axes_transform)
